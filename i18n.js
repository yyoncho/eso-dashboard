...
(function () {
  ...
  const SUB = [
    ...
    ['Пари от обмена (по интервали и натрупано)', 'Money from the exchange (per interval and cumulative)'],
    ...
    ['<b>Пари от обмена</b> = нетен поток (МВт) × ценова разлика (съсед − БГ) × продължителност на интервала, сумирано за ГР и РО.',
     '<b>Money from the exchange</b> = net flow (MW) × price difference (neighbour − BG) × interval duration, summed over GR and RO.'],
    ['<b>Реекспорт (транзит):</b> когато БГ едновременно внася от единия съсед и изнася към другия, припокриващият се обем се третира като преминаващ транзитно през БГ и се изважда — не се брои като печалба от собствена търговия на БГ.',
     '<b>Re-export (transit):</b> when BG simultaneously imports from one neighbour and exports to the other, the overlapping volume is treated as passing through BG in transit and is subtracted out — it does not count as BG\'s own trading profit.'],
    ['<b>4-часова батерия:</b> търси се неприпокриващ се 4-часов прозорец с най-ниска средна цена (зареждане) и 4-часов прозорец с най-висока средна цена (разреждане) в рамките на деня по БГ цената; приходът на MW е ориентировъчен, при 90% рундтрип ефективност.',
     '<b>4-hour battery:</b> a non-overlapping 4-hour window with the lowest average price (charging) and a 4-hour window with the highest average price (discharging) are found within the day using the BG price; the revenue per MW is indicative, at 90% round-trip efficiency.'],
    ...
    ['Наличен капацитет', 'Available capacity'],
    ...
    ['<b>Рента</b> (congestion rent) = нетен поток (МВт) × ценова разлика (съсед − БГ) × продължителност на интервала.',
     '<b>Rent</b> (congestion rent) = net flow (MW) × price difference (neighbour − BG) × interval duration.'],
    ...
    ['<b>Данни:</b> IBEX SDAC Market Coupling, обновявани дневно около 16:00 ч. българско време.',
     '<b>Data:</b> IBEX SDAC Market Coupling, updated daily around 16:00 Sofia time.'],
    ['<b>Обвързана цена</b> = разлика &le; 1 EUR/MWh между пазарите. <b>Наситен капацитет</b> = използван поток &ge; 90% от наличния.',
     '<b>Coupled price</b> = difference &le; 1 EUR/MWh between markets. <b>Saturated capacity</b> = used flow &ge; 90% of available.'],
    ...
    [' и БГ (', ' both BG ('],
    ['), и Гърция (', '), and Greece ('],
    [') паднаха до ниски цени през 10–16 ч, с обвързан пазар — регионален, не само локален излишък на слънчева енергия.',
     ') fell to low prices during 10:00–16:00, with a coupled market — a regional, not just local, solar surplus.'],
    ...
    ['% от интервалите на границата на пропускателната способност, при устойчива ценова разлика от ',
     '% of intervals at the transfer-capacity limit, with a persistent price gap of '],
    ...
    ['% от интервалите на границата на пропускателната способност, при ценова разлика от ',
     '% of intervals at the transfer-capacity limit, with a price gap of '],
    [' EUR/MWh.', ' EUR/MWh.'],
    ...
    ['% от интервалите — възможен недостиг на предлагане в Румъния, ограничен от преносната способност.',
     '% of intervals — a possible supply shortfall in Romania, constrained by transfer capacity.'],
