...
(function () {
  ...
  const SUB = [
    ...
    ['Трансгранични потоци', 'Cross-border flows'],
    ['Стойностите в EUR/h са ',
     'Values in EUR/h are '],
    ['оценка', 'an estimate'],
    [', не реален паричен поток: ', ', not a real money flow: '],
    ['износ', 'export'],
    [' = поток(MW) × текуща IBEX DA цена за БГ; ', ' = flow(MW) × the current IBEX DA price for BG; '],
    ['внос', 'import'],
    [' = −поток(MW) × същата цена. Цената на съседната страна (РО/ГР/РС/МК/ТР) не е използвана — при пазарно обвързване (market coupling) цените обичайно съвпадат, но при насищане на капацитета (congestion) те могат да се разминат и реалният приход/разход да е различен. Реален паричен поток зависи и от договорните условия на оператора, преносни такси и др. За реални цени по граница вижте ',
     ' = −flow(MW) × the same price. The neighbouring country\'s price (RO/GR/RS/MK/TR) is not used — under market coupling the prices are usually the same, but during congestion they can diverge and the real revenue/cost would differ. Real cash flow also depends on the operator\'s contract terms, transmission fees, etc. For real per-border prices see '],
    ['SDAC — Пазарно обвързване', 'SDAC — Market coupling'],
